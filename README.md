...
학번과 이름으로 자습 출석을 기록하고, 관리자 페이지에서 날짜별 출석 내역을 확인·수정·삭제할 수 있는 PHP 8.5 + SQLite 기반 출결 체크 시스템입니다.
...
환경 설정은 `data/config.php`에서 관리합니다. 앱 이름/버전, PHP 최소 버전, 시간대, DB 경로, 스키마 경로, 토큰 만료 시간, 비밀번호 최소 길이, 학번/이름 글자 수처럼 실제로 바꿀 가능성이 있는 값만 둡니다.
...
- 학번은 설정된 글자 수와 정확히 일치해야 하고, 이름은 설정된 최대 글자 수까지만 입력 가능
- 관리자 비밀번호 로그인과 토큰 기반 API 인증
- 관리자 대시보드, 목록, 수정/삭제, 비밀번호 변경을 AJAX로 처리
- 관리자 권한 없이 보호 페이지 접근 시 `/admin/?login=required`로 리다이렉션
