...
- GitHub 릴리즈에 SHA-256 digest가 있는 ZIP 자산이 있으면 해당 파일을
  우선 사용하고, 다운로드 직후 체크섬을 검증합니다.
- 체크섬 검증용 자산 이름은 `저장소명-태그.zip` 형식을 권장합니다.
  digest가 있는 ZIP 자산이 없으면 기존 태그 소스 ZIP을 사용합니다.
