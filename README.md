...
- 기본 정보: 앱 이름, 버전, 개발자 표시명, 시간대
- 저장소/업데이트: 공개 저장소 URL, 업데이트 확인용 GitHub `owner/repo`
...
- `/admin/system.php`: 초기화, 업데이트, 서버 정보 확인
- `/admin/password.php`: 관리자 비밀번호 변경

## 위치 인증

위치 인증을 켜면 출석 시 브라우저 위치 권한을 요청합니다. 출석 가능 반경 밖이거나 위치 권한을 사용할 수 없으면 관리자 승인 대기 상태로 기록할 수 있습니다.
...
로컬 정적 리소스는 앱 버전을 붙인 `?v=v1.6.9` 캐시버스터로 로드됩니다.
